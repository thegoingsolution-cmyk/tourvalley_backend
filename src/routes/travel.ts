...
/** 국내 단체 보험료: 어르신 구간(80+) → premium_rates 의 어르신플랜1(실속)/(표준) */
const resolveDomesticSeniorPlanTypeForGroup = (planType: string): string => {
  if (planType === '어르신플랜2' || planType === '어르신플랜' || planType === '어르신플랜1') {
    return '어르신플랜1(실속)';
  }
...
router.post('/api/travel/register-contract', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 5. 여행 계약 마일리지는 보험종료일+3일 경과 후 배치에서 적립 (scripts/accrueDeferredTravelMileage.ts)
